...
function getConfidenceBadge(confidence: number): {
  ...
  if (pct >= 80)
    return { label: `${pct}% High`, classes: "bg-red-100 text-red-700" };
  if (pct >= 60)
    return {
      label: `${pct}% Medium`,
...
export const MaintenancePredictionCard: React.FC<
  MaintenancePredictionCardProps
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-5 transition-shadow hover:shadow-md">
      <div className="flex items-start justify-between">
        ...
        <span
          className={`rounded-full px-2.5 py-0.5 font-body text-xs font-medium ${badge.classes}`}
        >
          {badge.label}
        </span>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        >
          <div
            ...
            style={{
              width: `${Math.min(100, Math.max(5, (1 - daysUntil / 365) * 100))}%`,
            }}
          />
        </div>
        <span
      ...
      <p className="mt-3 font-body text-sm text-gray-500">
        Predicted {getSystemLabel(prediction.systemType).toLowerCase()}{" "}
        maintenance needed based on historical data and system age analysis.
      </p>
    </div>
  );
};
