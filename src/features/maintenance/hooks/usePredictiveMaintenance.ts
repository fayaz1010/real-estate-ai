...
const MOCK_PROPERTIES = [
  { propertyId: 'prop-1', propertyTitle: '245 Oak Street Apartments', yearBuilt: 2005 },
  { propertyId: 'prop-2', propertyTitle: '180 Riverside Condos', yearBuilt: 1998 },
  { propertyId: 'prop-3', propertyTitle: '72 Maple Drive Townhomes', yearBuilt: 2012 },
];
...
export function usePredictiveMaintenance(): UsePredictiveMaintenanceReturn {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const propertyData: PropertyMaintenanceData[] = MOCK_PROPERTIES.map((prop) => {
        const records = MOCK_RECORDS.filter((r) => r.propertyId === prop.propertyId);
  ...
  const urgentPredictions = allPredictions.filter(
    (p) => new Date(p.predictedFailureDate) <= thirtyDaysFromNow && p.confidence >= 0.6,
  );

  // Aggregate risk by system type
  const systemRisks: SystemRisk[] = Object.values(MaintenanceSystemType).map((systemType) => {
    const systemPredictions = allPredictions.filter((p) => p.systemType === systemType);
