...
import {
  getPaymentMethods,
  type PaymentMethod as StripePaymentMethod,
} from "@/modules/payments/api/paymentService";
...
export const PaymentCollectionPage: React.FC = () => {
  ...
  const fetchPayments = async () => {
    ...
    setError(null);
    try {
      const { data } = await apiClient.get<{ data: ApiPayment[] }>(
  ...
  const filtered = useMemo(() => {
    let list = payments;
    if (activeTab !== "all") {
      const statusMap: Record<TabKey, DisplayStatus | null> = {
        all: null,
        received: "Received",
        pending: "Pending",
        overdue: "Overdue",
  ...
  const collectionRate =
    total > 0 ? Math.round((totalCollected / total) * 100) : 0;
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: "text-realestate-primary bg-realestate-accent/20",
    },
  ];
  ...
  if (loading) {
    return (
      <div
        className="section-container py-8 flex flex-col items-center justify-center min-h-[400px] gap-4"
        style={{ backgroundColor: "#FAF6F1" }}
      >
        <Loader2 className="w-8 h-8 animate-spin text-[#8B7355]" />
        <p
          className="text-sm"
          style={{ color: "#2D2A26", fontFamily: "Inter, sans-serif" }}
        >
          Loading payments...
        </p>
      </div>
    ...
    <div className="section-container py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <Link
            to="/dashboard"
            className="text-sm text-realestate-secondary hover:underline flex items-center gap-1 mb-2"
          >
            <ChevronRight className="w-4 h-4 rotate-180" /> Back to Dashboard
          </Link>
          <h1 className="text-display text-realestate-primary">
            Payment Collection
          </h1>
          <p className="text-gray-500 mt-1">
            Manage rent payments, methods, and automation.
          </p>
        </div>
        <button
                ...
                filtered.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 flex items-center gap-2">
                      <Users className="w-4 h-4 text-gray-400" /> {p.tenant}
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[p.status]}`}
                      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-elevated p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-heading text-lg font-semibold text-realestate-primary">
              Payment Methods
            </h2>
            <button
          ...
          <ul className="space-y-3" aria-label="Saved payment methods">
            {paymentMethods.length === 0 ? (
              ...
              paymentMethods.map((m) => (
                <li
                  key={m.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:border-realestate-secondary/40 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {m.type}
                        {m.last4 !== "N/A" ? ` ending ${m.last4}` : ""}
                      </p>
                    </div>
                  </div>
                  {m.isDefault && (
                    <span className="text-xs bg-realestate-accent/30 text-realestate-primary px-2 py-0.5 rounded-full font-medium">
                      Default
                    </span>
                  )}
                </li>
          ...
          <div className="flex items-center justify-between p-4 rounded-lg bg-gray-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Automatically charge tenants on their rent due date.
                </p>
              </div>
            </div>
            <button
            ...
            >
              <span
                className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${autoPayEnabled ? "translate-x-5" : ""}`}
              />
            </button>
          </div>
          <ul className="text-sm space-y-2 text-gray-600">
            <li className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-500" /> Reminders sent
              3 days before due date
            </li>
            <li className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-500" /> Late fee
              applied after 7-day grace period
            </li>
            <li className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-500" /> Receipts
              emailed automatically on payment
            </li>
          </ul>
        </div>
      </div>
