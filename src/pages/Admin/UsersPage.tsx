      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-text_primary font-display">User Management</h1>
              <p className="text-sm text-text_primary/60 font-body">Manage platform users and roles</p>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-primary/10 bg-primary/[0.02]">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-semibold text-text_primary/60 uppercase tracking-wider">Email</th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-text_primary/60 uppercase tracking-wider">Name</th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-text_primary/60 uppercase tracking-wider">Role</th>
                ...
                {loading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.id} className="hover:bg-primary/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-text_primary">{u.email}</td>
                      <td className="px-6 py-4 text-sm text-text_primary font-medium">
                        {u.firstName} {u.lastName}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${getRoleBadgeClasses(u.role)}`}>
                          {u.role.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${getStatusBadgeClasses(u.status)}`}>
                          {u.status.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-text_primary mb-1">First Name</label>
                  <Input
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-text_primary mb-1">Last Name</label>
                  <Input
                    required
              ...
              <div>
                <label className="block text-sm font-medium text-text_primary mb-1">Email</label>
                <Input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-text_primary mb-1">
                  Password {editingUser && <span className="text-text_primary/40">(leave blank to keep current)</span>}
                </label>
                <Input
                  type="password"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-text_primary mb-1">Role</label>
                  <select
                    value={formData.role}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData((f) => ({ ...f, status: e.target.value as AccountStatus }))}
                    className="w-full h-10 rounded-md border border-primary/30 bg-background px-3 text-sm text-text_primary"
                  >
                    {Object.values(AccountStatus).map((s) => (
                      <option key={s} value={s}>{s.replace('_', ' ')}</option>
