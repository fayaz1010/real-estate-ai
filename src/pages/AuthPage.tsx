...
const ROLE_OPTIONS: { value: UserRole; label: string; description: string }[] = [
  { value: UserRole.LANDLORD, label: 'Landlord', description: 'List and manage your rental properties' },
  { value: UserRole.TENANT, label: 'Tenant', description: 'Find and apply for rental homes' },
  { value: UserRole.AGENT, label: 'Agent', description: 'Connect buyers, sellers, and renters' },
];
...
const PLATFORM_BENEFITS = [
  {
    ...
    description: 'Get data-driven property valuations and market analysis powered by advanced AI.',
  },
  {
    icon: <Shield size={20} className="text-realestate-accent" />,
    title: 'Secure Transactions',
    description: 'End-to-end encrypted payments and document handling to protect every deal.',
  },
  {
    icon: <Zap size={20} className="text-realestate-accent" />,
    title: 'Streamlined Workflows',
    description: 'Automate lease management, rent collection, and maintenance requests.',
  },
];
...
const AuthPage: React.FC = () => {
  ...
  const getInitialTab = (): AuthTab => {
    if (location.pathname.includes('/auth/register') || location.pathname.includes('/register')) {
      return 'register';
    }
    return 'login';
  };
  ...
  const handleRegisterChange = (field: keyof RegisterFormData, value: string | boolean | UserRole) => {
  ...
  const passwordStrength = validators.getPasswordStrength(registerData.password);
        ...
        <div className="relative z-10 flex flex-col h-full px-10 xl:px-14 py-10">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <h2 className="text-3xl xl:text-4xl font-bold font-space-grotesk text-white leading-tight mb-4">
              Property Management,
              <br />
              <span className="text-realestate-accent">Powered by Intelligence</span>
            </h2>
            <p className="text-white/60 font-inter text-base mb-10 max-w-sm">
              Join thousands of property professionals who trust our platform to manage, analyze, and grow their portfolios.
            </p>
          ...
          <div className="mt-auto pt-8 border-t border-white/10">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/70 text-sm font-inter">12,000+ Users</span>
              </div>
              <div className="flex items-center gap-2">
                <Building size={16} className="text-realestate-accent" />
                <span className="text-white/70 text-sm font-inter">3,200+ Properties</span>
              </div>
              <div className="flex items-center gap-2">
                <Shield size={16} className="text-realestate-accent" />
                <span className="text-white/70 text-sm font-inter">256-bit SSL</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'login' && (
            <div className="animate-fade-in">
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-5" noValidate>
                {/* Email */}
                <div>
                  <label htmlFor="login-email" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                    Email Address
                  </label>
                  <div className="relative">
                    <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                    <input
                      id="login-email"
                      type="email"
                ...
                {/* Password */}
                <div>
                  <label htmlFor="login-password" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                    Password
                  </label>
                  <div className="relative">
                    <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                    <input
                      id="login-password"
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer" htmlFor="remember-me">
                    ...
                    />
                    <span className="text-sm text-realestate-secondary font-inter">Remember me</span>
                  </label>
                  <Link
              ...
              <div className="my-6 flex items-center gap-3">
                ...
                <span className="text-xs text-realestate-secondary font-inter uppercase tracking-wide">or continue with</span>
                <div className="flex-1 h-px bg-gray-200" />
              </div>
            ...
            <div className="animate-fade-in">
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-5" noValidate>
                    ...
                    {ROLE_OPTIONS.map((role) => (
                      ...
                      >
                        <p className={`text-sm font-semibold font-inter ${
                          registerData.role === role.value ? 'text-realestate-primary' : 'text-realestate-secondary'
                        }`}>
                          {role.label}
                        </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="reg-first-name" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                      First Name
                    </label>
                    <div className="relative">
                      <UserIcon size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                      <input
                        id="reg-first-name"
                        type="text"
                  ...
                  <div>
                    <label htmlFor="reg-last-name" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                      Last Name
                    </label>
                    <input
                      id="reg-last-name"
                ...
                {/* Email */}
                <div>
                  <label htmlFor="reg-email" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                    Email Address
                  </label>
                  <div className="relative">
                    <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                ...
                {/* Password */}
                <div>
                  <label htmlFor="reg-password" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                    Password
                  </label>
                  <div className="relative">
                    <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                ...
                {/* Confirm Password */}
                <div>
                  <label htmlFor="reg-confirm-password" className="block text-sm font-medium text-realestate-primary mb-1.5 font-inter">
                    Confirm Password
                  </label>
                  <div className="relative">
                    <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-realestate-secondary" />
                ...
                {/* Terms */}
                <div>
                  <label className="flex items-start gap-3 cursor-pointer" htmlFor="accept-terms">
                    <input
                      id="accept-terms"
                      type="checkbox"
                      checked={registerData.acceptedTerms}
                      onChange={(e) => handleRegisterChange('acceptedTerms', e.target.checked)}
                    ...
                    <span className="text-sm text-realestate-secondary font-inter leading-snug">
                      I agree to the{' '}
                      <Link to="/terms" className="text-realestate-accent hover:text-amber-600 font-medium transition-colors">
                        Terms of Service
                      </Link>{' '}
                      and{' '}
                      <Link to="/privacy" className="text-realestate-accent hover:text-amber-600 font-medium transition-colors">
                        Privacy Policy
                      </Link>
                    </span>
                  </label>
              ...
              <div className="my-6 flex items-center gap-3">
                <div className="flex-1 h-px bg-gray-200" />
                <span className="text-xs text-realestate-secondary font-inter uppercase tracking-wide">or sign up with</span>
                <div className="flex-1 h-px bg-gray-200" />
              </div>
