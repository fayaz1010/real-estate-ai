...
const PLAN_PRICE_IDS: Record<Exclude<Plan, 'FREE' | 'ENTERPRISE'>, { monthly: string; annual: string }> = {
  STARTER: {
...
export async function createCustomer(email: string, name: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Failed to create Stripe customer: ${res.statusText}`);
...
export async function createSubscription(
  ...
  if (!res.ok) throw new Error(`Failed to create Stripe subscription: ${res.statusText}`);
  const data = await res.json();
  return data.id as string;
}
...
export async function cancelSubscription(subscriptionId: string): Promise<void> {
  ...
  if (!res.ok) throw new Error(`Failed to cancel Stripe subscription: ${res.statusText}`);
}
...
export async function getInvoiceHistory(customerId: string): Promise<StripeInvoice[]> {
