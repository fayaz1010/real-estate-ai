...
const defaultQuickReplies: QuickReplyOption[] = [
  { id: 'qr-1', text: 'Submit maintenance request', icon: '🔧', category: 'maintenance' },
  { id: 'qr-2', text: 'Payment received, thanks!', icon: '💰', category: 'payment' },
  { id: 'qr-3', text: 'Thanks for the update!', icon: '👍', category: 'general' },
  { id: 'qr-4', text: 'When is this scheduled?', icon: '📅', category: 'general' },
  { id: 'qr-5', text: 'Issue is still ongoing', icon: '⚠️', category: 'maintenance' },
...
export async function sendMessage(
  ...
  content: string,
  options?: { isQuickReply?: boolean; replyTo?: string; voiceNoteUrl?: string; voiceDuration?: number }
...
export async function uploadVoiceNote(_blob: Blob): Promise<{ url: string; duration: number }> {
...
export function getQuickReplies(category?: 'maintenance' | 'payment' | 'general'): QuickReplyOption[] {
  if (category) {
