...
import {
  ...
  getQuickReplies,
  uploadVoiceNote,
} from "../../services/mobileMessagingService";
...
const MobileConversation: React.FC<MobileConversationProps> = ({
  ...
  const handleQuickReply = async (option: { text: string }) => {
    const msg = await sendMessage(conversationId, option.text, {
      isQuickReply: true,
    });
    setMessages((prev) => [...prev, msg]);
          ...
          return (
            ...
            >
              ...
              >
                ...
                <p
                  className={`mt-1 text-right text-[10px] ${isMine ? "text-white/60" : "text-gray-400"}`}
                >
