...
const featureRows: FeatureRow[] = [
  { name: 'Units included', starter: 'Up to 25', professional: 'Up to 100', business: 'Up to 500', enterprise: 'Unlimited' },
  { name: 'AI insights', starter: 'Limited', professional: 'Full', business: 'Advanced', enterprise: 'Custom AI' },
  { name: 'Tenant screening', starter: true, professional: true, business: true, enterprise: true },
  { name: 'Rent collection', starter: true, professional: true, business: true, enterprise: true },
  { name: 'Predictive maintenance', starter: false, professional: true, business: true, enterprise: true },
  { name: 'Smart rent pricing', starter: false, professional: true, business: true, enterprise: true },
  { name: 'Automated workflows', starter: false, professional: false, business: true, enterprise: true },
  { name: 'Financial reporting', starter: false, professional: true, business: true, enterprise: true },
  { name: 'API access', starter: false, professional: true, business: true, enterprise: true },
  { name: 'Portfolio optimization', starter: false, professional: false, business: true, enterprise: true },
  { name: 'Custom workflows', starter: false, professional: false, business: true, enterprise: true },
  { name: 'White-label options', starter: false, professional: false, business: true, enterprise: true },
  ...
  { name: 'SLA guarantees', starter: false, professional: false, business: false, enterprise: true },
  { name: 'Support', starter: 'Email', professional: 'Priority', business: 'Priority', enterprise: '24/7 Dedicated' },
];
...
function BillingToggle({ interval, onToggle }: { interval: BillingInterval; onToggle: () => void }) {
  const isAnnual = interval === 'yearly';
  return (
    <div className="flex items-center justify-center gap-4 animate-fade-in">
      <span className={`font-body text-sm font-medium transition-colors duration-200 ${!isAnnual ? 'text-realestate-primary' : 'text-gray-400'}`}>
        Monthly
      </span>
      <button
        type="button"
...
function PricingCard({ plan, interval, onSelect }: { plan: Plan; interval: BillingInterval; onSelect: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-2">
        <div
          className={`flex items-center justify-center w-10 h-10 rounded-lg ${
            plan.highlighted ? 'bg-realestate-accent/10 text-realestate-accent' : 'bg-realestate-primary/5 text-realestate-primary'
          }`}
        >
          {PLAN_ICONS[plan.id]}
        </div>
      ...
      <p className="font-body text-sm text-gray-500 mb-6">
        {plan.id === 'starter' && 'Perfect for small landlords getting started with AI management.'}
        {plan.id === 'professional' && 'For growing portfolios that need powerful AI tools.'}
        {plan.id === 'business' && 'For property management firms scaling operations.'}
        {plan.id === 'enterprise' && 'Tailored solutions for large-scale property operations.'}
      </p>
...
function FeatureCell({ value }: { value: boolean | string }) {
  if (typeof value === 'string') {
    return <span className="font-body text-sm text-gray-700 font-medium">{value}</span>;
  }
  return value ? (
...
function FAQItem({ faq }: { faq: FAQ }) {
  ...
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      >
        <span className="text-heading text-base text-realestate-primary pr-4">{faq.question}</span>
        {open ? (
      ...
      <div className={`overflow-hidden transition-all duration-300 ${open ? 'max-h-96 pb-5' : 'max-h-0'}`}>
        <p className="font-body text-sm text-gray-600 leading-relaxed px-2">{faq.answer}</p>
      </div>
    </div>
  );
}
    ...
    <div className="pt-24 pb-20 bg-white">
      ...
      <section className="section-container text-center mb-12 animate-fade-in">
        ...
        <p className="font-body text-lg text-realestate-secondary max-w-2xl mx-auto">
          AI that actively manages your properties — predicting maintenance issues, optimizing rent prices,
          screening tenants, and reducing vacancy — so you manage more units with less effort.
        </p>
      </section>

      {/* Billing Toggle */}
      <section className="section-container mb-14">
        <BillingToggle interval={interval} onToggle={() => setInterval(interval === 'monthly' ? 'yearly' : 'monthly')} />
      </section>
      ...
      <section className="section-container mb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 xl:gap-4 items-start">
          {PLANS.map((plan) => (
            <PricingCard key={plan.id} plan={plan} interval={interval} onSelect={handleSelectPlan} />
          ))}
        </div>
      </section>
      ...
      <section className="section-container mb-24 animate-fade-in">
        <h2 className="text-display text-3xl text-center text-realestate-primary mb-10">Compare Plans</h2>
          ...
          <table className="w-full border-collapse" role="table">
            <thead>
              <tr className="border-b-2 border-gray-200">
                <th className="text-left py-4 pr-4 text-heading text-sm text-realestate-primary w-1/5">Feature</th>
                <th className="text-center py-4 px-4 text-heading text-sm text-realestate-primary w-1/5">Starter</th>
                <th className="text-center py-4 px-4 text-heading text-sm text-realestate-accent w-1/5">Professional</th>
                <th className="text-center py-4 px-4 text-heading text-sm text-realestate-primary w-1/5">Business</th>
                <th className="text-center py-4 px-4 text-heading text-sm text-realestate-primary w-1/5">Enterprise</th>
              </tr>
            </thead>
            <tbody>
              {featureRows.map((row) => (
                <tr key={row.name} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                  <td className="py-3.5 pr-4 font-body text-sm text-gray-700">{row.name}</td>
                  <td className="py-3.5 px-4 text-center"><FeatureCell value={row.starter} /></td>
                  <td className="py-3.5 px-4 text-center bg-realestate-accent/[0.03]"><FeatureCell value={row.professional} /></td>
          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`rounded-xl p-5 ${
                plan.highlighted ? 'border-2 border-realestate-accent bg-white shadow-realestate-md' : 'border border-gray-200 bg-white shadow-realestate-sm'
              }`}
            >
              <h3 className="text-heading text-lg text-realestate-primary mb-4 flex items-center gap-2">
                {PLAN_ICONS[plan.id]}
                {plan.name}
                {plan.highlighted && <span className="text-xs font-bold text-realestate-accent">(Most Popular)</span>}
              </h3>
                ...
                {featureRows.map((row) => {
                  ...
                  return (
                    <li key={row.name} className="flex items-center justify-between">
                      <span className="font-body text-sm text-gray-600">{row.name}</span>
                      <span className="flex-shrink-0 ml-3"><FeatureCell value={val as boolean | string} /></span>
                    </li>
                  );
                })}
              </ul>
      ...
      <section className="section-container mb-24 animate-fade-in">
        <div className="text-center mb-10">
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <span className="font-body text-sm font-semibold text-realestate-accent uppercase tracking-wide">FAQ</span>
          </div>
          <h2 className="text-display text-3xl text-realestate-primary">Frequently Asked Questions</h2>
        </div>
      ...
      <section className="section-container animate-fade-in">
        <div className="relative overflow-hidden rounded-2xl bg-realestate-primary px-6 py-14 sm:px-12 sm:py-20 text-center">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-display text-3xl sm:text-4xl text-white mb-4">Still Have Questions?</h2>
            <p className="font-body text-lg text-gray-300 max-w-xl mx-auto mb-8">
              Our team is here to help you find the right AI property management software plan for your portfolio.
            </p>
            <Link
              to="/contact"
