...
export default function CheckoutForm({ plan, interval }: CheckoutFormProps) {
  ...
  const totalPrice = interval === 'yearly' ? plan.yearlyPrice : plan.monthlyPrice;
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-200 p-6 mb-8">
        <h3 className="text-heading text-lg text-realestate-primary mb-4">Order Summary</h3>
        <div className="space-y-3">
          <div className="flex justify-between font-body text-sm">
            <span className="text-gray-600">{plan.name} Plan</span>
            <span className="text-realestate-primary font-medium">{formatPrice(monthlyEquiv)}/mo</span>
          </div>
          <div className="flex justify-between font-body text-sm">
            <span className="text-gray-600">Billing cycle</span>
            <span className="text-realestate-primary font-medium capitalize">{interval}</span>
          </div>
          {interval === 'yearly' && (
            <div className="flex justify-between font-body text-sm">
          ...
          <div className="border-t border-gray-200 pt-3">
            <div className="flex justify-between">
              ...
              <span className="font-body text-base font-semibold text-realestate-primary">{formatPrice(totalPrice)}</span>
            </div>
          </div>
        </div>
        <p className="font-body text-xs text-gray-400 mt-4">
          Includes a 14-day free trial. You won&apos;t be charged until the trial ends.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-center gap-2 mt-4">
          ...
          <p className="font-body text-xs text-gray-400">Secured by Stripe. Your payment info is never stored on our servers.</p>
        </div>
      </form>
