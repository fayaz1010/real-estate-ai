...
import {
  ...
  export function generateAvailableSlots(params: {
    ...
    const now = new Date();
    const minBookingTime = new Date(now.getTime() + minAdvanceNotice * 60 * 60 * 1000);
      ...
      .map((slot) => {
        ...
        if (preferredTimes) {
          ...
          if (preferredTimes.includes('morning') && hour >= 8 && hour < 12) {
            score += 5;
          } else if (preferredTimes.includes('afternoon') && hour >= 12 && hour < 17) {
            score += 5;
          } else if (preferredTimes.includes('evening') && hour >= 17 && hour < 21) {
            score += 5;
          }
        }
