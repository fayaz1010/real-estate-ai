...
export const fetchRecurringSchedules = createAsyncThunk(
  ...
  async (propertyId: string) => {
    const response =
      await availabilityService.getRecurringSchedules(propertyId);
    return response;
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createRecurringSchedule.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.error.message || "Failed to create recurring schedule";
      })
      ...
      .addCase(fetchRecurringSchedules.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.error.message || "Failed to fetch recurring schedules";
      })
...
export const selectBlackoutDates = (state: RootState) =>
  state.availability.blackoutDates;
