...
export const updateInspectionStatus = createAsyncThunk(
  ...
  async ({ id, status, notes }: { id: string; status: InspectionStatus; notes?: string }) => {
    const response = await inspectionService.updateInspection(id, { status, landlordNotes: notes });
    return response;
  }
);
...
export const rescheduleInspection = createAsyncThunk(
  ...
  async ({
    ...
    const response = await inspectionService.rescheduleInspection(id, newDate, newTime, reason);
    return response;
  }
);
...
export const fetchAvailableSlots = createAsyncThunk(
  ...
  async (params: {
    ...
    const response = await inspectionService.getAvailableSlots(params.propertyId, params.startDate);
    return response;
  }
);
...
export const checkInInspection = createAsyncThunk(
  ...
  async ({
    ...
    const response = await inspectionService.checkInInspection(id, { location, photo });
    return response;
  }
);
...
export const checkOutInspection = createAsyncThunk(
  ...
  async ({
    ...
    const response = await inspectionService.checkOutInspection(id, { feedback });
    return response;
  }
);
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateInspectionStatus.rejected, (state, action) => {
        state.isUpdatingInspection = false;
        state.error = action.error.message || 'Failed to update inspection status';
      });
    ...
    builder
      .addCase(rescheduleInspection.fulfilled, (state, action) => {
        const index = state.inspections.findIndex(i => i.id === action.payload.id);
        if (index !== -1) {
          state.inspections[index] = action.payload;
        }
    ...
    // Check-in inspection
    builder
      .addCase(checkInInspection.fulfilled, (state, action) => {
        const index = state.inspections.findIndex(i => i.id === action.payload.id);
        if (index !== -1) {
          state.inspections[index] = action.payload;
        }
    ...
    builder
      .addCase(checkOutInspection.fulfilled, (state, action) => {
        const index = state.inspections.findIndex(i => i.id === action.payload.id);
        if (index !== -1) {
          state.inspections[index] = action.payload;
        }
...
export const selectCurrentInspection = (state: { inspections: InspectionState }) =>
...
export const selectInspectionLoading = (state: { inspections: InspectionState }) =>
...
export const selectInspectionError = (state: { inspections: InspectionState }) =>
  state.inspections.error;

export const selectInspectionFilters = (state: { inspections: InspectionState }) =>
  state.inspections.filters;

export const selectInspectionPagination = (state: { inspections: InspectionState }) => ({
...
export const selectInspectionUI = (state: { inspections: InspectionState }) => ({
