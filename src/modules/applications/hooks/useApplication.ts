...
export const useApplication = (applicationId?: string) => {
  ...
  useEffect(() => {
    if (applicationId && (!currentApplication || currentApplication.id !== applicationId)) {
      dispatch(fetchApplication(applicationId));
    }
  }, [applicationId, dispatch, currentApplication]);

  // Create new application
  const create = useCallback(async (propertyId: string) => {
  ...
  const update = useCallback(async (id: string, data: any) => {
    const result = await dispatch(updateApplication({ applicationId: id, data }));
    return result.payload as Application;
  }, [dispatch]);
  ...
  const approve = useCallback(async (id: string, conditions?: string[]) => {
    const result = await dispatch(approveApplication({ applicationId: id, conditions }));
    return result.payload as Application;
  }, [dispatch]);

  // Reject application (landlord)
  const reject = useCallback(async (id: string, reason: string) => {
    const result = await dispatch(rejectApplication({ applicationId: id, reason }));
    return result.payload as Application;
  }, [dispatch]);
  ...
  const fetchMy = useCallback(async (filters?: ApplicationFilters) => {
    const result = await dispatch(fetchMyApplications(filters));
    return result.payload as Application[];
  }, [dispatch]);

  // Set current application
  const setCurrent = useCallback((application: Application) => {
