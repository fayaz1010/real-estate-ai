...
export const useApplicationForm = () => {
  ...
  const [localFormData, setLocalFormData] = useState<Partial<ApplicationFormData>>(currentFormData);
  ...
  const updateNestedField = useCallback((
    parent: keyof ApplicationFormData,
    field: string,
    value: any
  ) => {
    setLocalFormData(prev => ({
  ...
  const addArrayItem = useCallback((field: keyof ApplicationFormData, item: any) => {
  ...
  // Remove array item
  const removeArrayItem = useCallback((field: keyof ApplicationFormData, index: number) => {
    const currentArray = [...((localFormData[field] as any[]) || [])];
