...
import { 
   ...
   */
  export const calculateApplicationScore = (application: Partial<Application>): number => {
   ...
   */
  export const calculateScoreBreakdown = (application: Partial<Application>): ScoreBreakdown => {
  ...
  const calculateRentalScore = (application: Partial<Application>) => {
    ...
    const hasNegativeReasons = rentalHistory.some(entry => 
  ...
  const calculateBackgroundScore = (application: Partial<Application>) => {
    ...
    const hasFelonies = backgroundCheck.criminalRecords.some(r => r.type === 'felony');
  ...
  const calculateCompletenessScore = (application: Partial<Application>) => {
    ...
    totalFields += 1;
    if (application.employment && application.employment.length > 0) completedFields++;
    ...
    totalFields += 1;
    if (application.rentalHistory && application.rentalHistory.length > 0) completedFields++;
  
    // References (at least 2)
    totalFields += 1;
    if (application.references && application.references.length >= 2) completedFields++;
   ...
   */
  export const qualifiesForInstantApproval = (application: Partial<Application>): boolean => {
