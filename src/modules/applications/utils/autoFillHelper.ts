...
import { 
  ...
  export interface AutoFillSource {
    source: 'previous_application' | 'user_profile' | 'browser' | 'document_parse' | 'third_party';
    confidence: number;
  ...
  export const getFieldSuggestions = (
    ...
    switch (fieldName) {
      ...
      case 'jobTitle':
        suggestions.push(...getSuggestedJobTitles(partialValue, context.employment?.[0]?.employerName));
        break;
  ...
  const mergeEmploymentFromPaystub = (
    ...
    const existingIndex = current.findIndex(
      e => e.employerName?.toLowerCase() === paystubData.employerName?.toLowerCase()
    );
  ...
  const determinePayFrequency = (paystubData: any): 'weekly' | 'biweekly' | 'monthly' => {
  ...
  const getSuggestedEmployers = (partial: string): string[] => {
    const employers = [
      ...
      'Tesla', 'Walmart', 'Target', 'Starbucks', 'McDonald\'s'
    ];
  ...
  const getSuggestedJobTitles = (partial: string, employer?: string): string[] => {
    const titles = [
      'Software Engineer', 'Project Manager', 'Sales Manager',
      'Marketing Manager', 'Accountant', 'Teacher', 'Nurse',
