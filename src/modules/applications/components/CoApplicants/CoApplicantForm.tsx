...
import { CoApplicant, PersonalInfo, EmploymentInfo, IncomeInfo } from '../../types/application.types';
...
const CoApplicantForm: React.FC<CoApplicantFormProps> = ({ applicationId, coApplicant, onSave }) => {
  const [activeSection, setActiveSection] = useState<'personal' | 'employment' | 'income'>('personal');
  const [personalInfo, setPersonalInfo] = useState<Partial<PersonalInfo>>(coApplicant.personalInfo || {});
  const [employment, setEmployment] = useState<Partial<EmploymentInfo>[]>(coApplicant.employment || [{}]);
  const [income, setIncome] = useState<Partial<IncomeInfo>[]>(coApplicant.income || [{}]);
  const [saving, setSaving] = useState(false);

  const handleSave = async () => {
    setSaving(true);
    try {
      await applicationService.updateCoApplicant(applicationId, coApplicant.id, {
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updated[index] = { ...updated[index], source: e.target.value as any };
                      setIncome(updated);
                    }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  >
