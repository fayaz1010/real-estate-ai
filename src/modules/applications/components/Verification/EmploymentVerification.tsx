...
import { Briefcase, CheckCircle, Clock, Mail, Phone, AlertCircle } from 'lucide-react';
...
const EmploymentVerification: React.FC<EmploymentVerificationProps> = ({ applicationId }) => {
  const { employmentStatus, startEmploymentVerification, loading } = useVerification(applicationId);
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6">
        <div className="flex items-center mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-green-900">Employment Verified</h3>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Employment Verification</h2>
      ...
      {employmentStatus !== 'not_started' && (
        <div className={`rounded-lg p-4 flex items-center ${
          employmentStatus === 'pending' || employmentStatus === 'in_progress' ? 'bg-yellow-50 border border-yellow-200' :
          employmentStatus === 'failed' ? 'bg-red-50 border border-red-200' :
          'bg-gray-50 border border-gray-200'
        }`}>
          {(employmentStatus === 'pending' || employmentStatus === 'in_progress') && <Clock className="w-6 h-6 text-yellow-600 mr-3" />}
          {employmentStatus === 'failed' && <AlertCircle className="w-6 h-6 text-red-600 mr-3" />}
          <div>
            <p className="font-medium text-gray-900 capitalize">
              {employmentStatus.replace('_', ' ')}
            </p>
            <p className="text-sm text-gray-600">
              {employmentStatus === 'pending' && 'Waiting for employer response'}
              {employmentStatus === 'in_progress' && 'Verification in progress'}
              {employmentStatus === 'failed' && 'Verification could not be completed'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-start mb-4">
          ...
          <div className={`px-3 py-1 rounded-full text-xs font-medium ${
            currentEmployer.verificationStatus === 'verified' ? 'bg-green-100 text-green-800' :
            currentEmployer.verificationStatus === 'pending' ? 'bg-yellow-100 text-yellow-800' :
            'bg-gray-100 text-gray-800'
          }`}>
            {currentEmployer.verificationStatus?.replace('_', ' ') || 'Not Verified'}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-gray-900">{currentEmployer.startDate}</p>
          </div>
          {!currentEmployer.isCurrent && (
            <div>
              <p className="text-gray-600">End Date</p>
              <p className="font-medium text-gray-900">{currentEmployer.endDate}</p>
            </div>
          )}
          <div>
      ...
      {(currentEmployer.supervisorName || currentEmployer.supervisorPhone || currentEmployer.supervisorEmail) && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h4 className="font-semibold text-blue-900 mb-3">Supervisor Contact</h4>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-3">
          <div className="flex items-start">
            ...
            <div>
              <p className="font-medium text-gray-900">We'll Contact Your Employer</p>
              <p className="text-sm text-gray-600">
          ...
          <div className="flex items-start">
            ...
            <div>
              <p className="font-medium text-gray-900">Supervisor Confirmation</p>
              <p className="text-sm text-gray-600">
      ...
      <button
        ...
        disabled={loading || currentEmployer.verificationStatus === 'verified' || currentEmployer.verificationStatus === 'pending'}
