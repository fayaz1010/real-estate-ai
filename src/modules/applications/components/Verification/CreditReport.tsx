...
import { CreditCard, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Download } from 'lucide-react';
...
const CreditReport: React.FC<CreditReportProps> = ({ application }) => {
  ...
  const getScoreColor = (score: number) => {
    if (score >= 750) return { bg: 'bg-green-50', border: 'border-green-500', text: 'text-green-700', rating: 'Excellent' };
    if (score >= 700) return { bg: 'bg-blue-50', border: 'border-blue-500', text: 'text-blue-700', rating: 'Good' };
    if (score >= 650) return { bg: 'bg-yellow-50', border: 'border-yellow-500', text: 'text-yellow-700', rating: 'Fair' };
    if (score >= 600) return { bg: 'bg-orange-50', border: 'border-orange-500', text: 'text-orange-700', rating: 'Poor' };
    return { bg: 'bg-red-50', border: 'border-red-500', text: 'text-red-700', rating: 'Very Poor' };
  };
    ...
    <div className="space-y-6">
      ...
      <div className={`${colors.bg} border-2 ${colors.border} rounded-lg p-6`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm font-medium text-gray-700 mb-1">Credit Score</p>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Credit Report Details</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="font-medium text-gray-900 capitalize">{creditCheck.provider}</p>
          </div>
          <div>
      ...
      {creditCheck.alerts && creditCheck.alerts.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          <div className="flex items-start mb-3">
            ...
            <div>
              <h4 className="font-semibold text-yellow-900 mb-2">Credit Alerts</h4>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h4 className="font-semibold text-blue-900 mb-3">What This Score Means</h4>
        <div className="space-y-4 text-sm text-blue-800">
          {creditCheck.score >= 750 && (
            <div>
              <p className="font-medium mb-1">Excellent Credit (750+)</p>
              <p>This applicant has excellent credit and is likely to be approved for most rentals. They have a strong history of on-time payments and responsible credit use.</p>
            </div>
          )}
          {creditCheck.score >= 700 && creditCheck.score < 750 && (
            <div>
              <p className="font-medium mb-1">Good Credit (700-749)</p>
              <p>This applicant has good credit and should qualify for most rentals. They demonstrate reliable payment history with minor blemishes if any.</p>
            </div>
          )}
          {creditCheck.score >= 650 && creditCheck.score < 700 && (
            <div>
              <p className="font-medium mb-1">Fair Credit (650-699)</p>
              <p>This applicant has fair credit. They may qualify but might need additional verification or a higher deposit. Review other factors carefully.</p>
            </div>
          )}
          {creditCheck.score >= 600 && creditCheck.score < 650 && (
            <div>
              <p className="font-medium mb-1">Poor Credit (600-649)</p>
              <p>This applicant has poor credit. Consider requiring a co-signer, higher deposit, or additional verification before approval.</p>
            </div>
          )}
          {creditCheck.score < 600 && (
            <div>
              <p className="font-medium mb-1">Very Poor Credit (Below 600)</p>
              <p>This applicant has significant credit challenges. Strongly consider requiring a co-signer or guarantor, and verify other qualifications thoroughly.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h4 className="font-semibold text-gray-900 mb-4">Credit Score Ranges</h4>
        <div className="space-y-3">
          <div className="flex items-center">
            ...
            <div className="flex-1 h-8 bg-green-100 rounded flex items-center px-3">
              <span className="text-sm font-medium text-green-800">Exceptional</span>
            </div>
          </div>
          <div className="flex items-center">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <p className="text-xs text-gray-600">
          <strong>Note:</strong> Credit scores are provided by {creditCheck.provider} and are one factor in the application decision. Other factors including income, employment, and rental history are also considered. This information is confidential and should only be used for application evaluation purposes.
        </p>
      </div>
    </div>
  );
