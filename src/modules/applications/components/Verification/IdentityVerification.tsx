...
import { Camera, Upload, CheckCircle, XCircle, Loader, User } from 'lucide-react';
...
const IdentityVerification: React.FC<IdentityVerificationProps> = ({ applicationId }) => {
  const { identityStatus, startIdentityVerification, loading } = useVerification(applicationId);
  const { formData } = useApplicationForm();
  
  const [step, setStep] = useState<'upload' | 'selfie' | 'processing' | 'complete'>('upload');
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
            ...
            ['selfie', 'processing', 'complete'].includes(step) ? 'bg-green-600 text-white' : 
        ...
        <div className="flex-1 h-1 bg-gray-200 mx-4">
          <div className={`h-1 transition-all ${
            ['selfie', 'processing', 'complete'].includes(step) ? 'bg-blue-600 w-full' : 'w-0'
          }`} />
        </div>
        <div className="flex items-center">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
            step === 'selfie' ? 'bg-blue-600 text-white' : 
            ['processing', 'complete'].includes(step) ? 'bg-green-600 text-white' : 
            'bg-gray-200 text-gray-600'
          }`}>
            2
          </div>
        ...
        <div className="flex-1 h-1 bg-gray-200 mx-4">
          <div className={`h-1 transition-all ${
            ['processing', 'complete'].includes(step) ? 'bg-blue-600 w-full' : 'w-0'
          }`} />
        </div>
        <div className="flex items-center">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
              {idFrontImage ? (
                <div className="relative">
                  <img src={idFrontImage} alt="ID Front" className="w-full rounded" />
                  <button
                ...
                <label className="cursor-pointer">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => e.target.files && handleFileUpload(e.target.files[0], 'front')}
                    className="hidden"
                  />
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
              {idBackImage ? (
                <div className="relative">
                  <img src={idBackImage} alt="ID Back" className="w-full rounded" />
                  <button
                ...
                <label className="cursor-pointer">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => e.target.files && handleFileUpload(e.target.files[0], 'back')}
                    className="hidden"
                  />
                  <Upload className="w-12 h-12 text-gray-400 mx-auto mb-3" />
        ...
        <div className="space-y-6">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              Take a clear selfie facing the camera. Make sure your face is well-lit and clearly visible.
            </p>
          </div>
