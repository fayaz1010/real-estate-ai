...
import { Clock, CheckCircle, XCircle, FileText, User, Shield, DollarSign } from 'lucide-react';
...
const ApplicationTimeline: React.FC<ApplicationTimelineProps> = ({ application }) => {
  const events = [
    ...
    ...(application.creditCheck.pulledAt ? [{
      ...
      icon: Shield,
      color: application.creditCheck.score >= 700 ? 'green' : application.creditCheck.score >= 600 ? 'yellow' : 'red',
    }] : []),
    ...(application.backgroundCheck.completedAt ? [{
      id: 6,
      type: 'background_check',
      title: 'Background Check Completed',
      description: application.backgroundCheck.criminalRecords.length === 0 && application.backgroundCheck.evictionRecords.length === 0
        ? 'No records found'
      ...
      icon: Shield,
      color: application.backgroundCheck.criminalRecords.length === 0 ? 'green' : 'red',
    }] : []),
    ...(application.reviewedAt ? [{
      id: 7,
      type: application.status === 'approved' ? 'approved' : application.status === 'rejected' ? 'rejected' : 'reviewed',
      title: application.status === 'approved' ? 'Application Approved' : application.status === 'rejected' ? 'Application Rejected' : 'Application Reviewed',
      ...
      icon: application.status === 'approved' ? CheckCircle : application.status === 'rejected' ? XCircle : FileText,
          ...
          {events.map((event, index) => {
            ...
            return (
              <div key={event.id} className="relative flex items-start">
                {/* Icon */}
                <div className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-full ${colors.bg} border-4 border-white`}>
                  <Icon className={`w-6 h-6 ${colors.text}`} />
                </div>

                {/* Content */}
                <div className="ml-6 flex-1">
                  <div className={`bg-white border ${colors.border} rounded-lg p-4 shadow-sm`}>
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800">
              Application is currently <span className="font-medium capitalize">{application.status.replace('_', ' ')}</span>
            </p>
