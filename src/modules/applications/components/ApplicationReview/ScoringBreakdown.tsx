...
const ScoringBreakdown: React.FC<ScoringBreakdownProps> = ({ application }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Overall Score */}
      <div className={`rounded-lg border-2 p-6 ${getScoreColor(application.score)}`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-semibold mb-1">Overall Application Score</h3>
      ...
      <div>
        ...
        <div className="space-y-4">
          {scoreCategories.map((category) => {
            const weightedScore = Math.round(category.data.score * category.data.weight);
            const maxWeightedScore = Math.round(100 * category.data.weight);

            return (
              <div key={category.key} className="bg-white border border-gray-200 rounded-lg p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="text-right ml-4">
                    <div className={`text-2xl font-bold ${
                      category.data.score >= 90 ? 'text-green-600' :
                      category.data.score >= 75 ? 'text-blue-600' :
                      category.data.score >= 60 ? 'text-yellow-600' :
                      'text-red-600'
                    }`}>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="text-sm text-blue-800 space-y-2">
          <p>
            The application score is calculated using a weighted algorithm that considers multiple factors:
          </p>
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li><strong>Credit Score (30%):</strong> Higher credit scores indicate financial responsibility</li>
            <li><strong>Income Ratio (25%):</strong> Monthly income should be 3x the rent ideally</li>
            <li><strong>Employment (15%):</strong> Longer, stable employment history is preferred</li>
            <li><strong>Rental History (15%):</strong> Positive references and no evictions</li>
            <li><strong>Background Check (10%):</strong> Clean criminal and eviction record</li>
            <li><strong>Completeness (5%):</strong> Fully completed application with all documents</li>
          </ul>
        </div>
      </div>
      ...
      <div className={`rounded-lg border-2 p-4 ${
        application.score >= 90 ? 'bg-green-50 border-green-200' :
        application.score >= 75 ? 'bg-blue-50 border-blue-200' :
        application.score >= 60 ? 'bg-yellow-50 border-yellow-200' :
        ...
        <p className="text-sm">
          {application.score >= 90 && (
            <span className="text-green-800">
              <strong>Strongly Recommended:</strong> This is an excellent applicant with high scores across all categories. They meet or exceed all qualification criteria.
            </span>
          )}
          {application.score >= 75 && application.score < 90 && (
            <span className="text-blue-800">
              <strong>Recommended:</strong> This is a good applicant who meets qualification standards. Consider approving with standard lease terms.
            </span>
          )}
          {application.score >= 60 && application.score < 75 && (
            <span className="text-yellow-800">
              <strong>Consider Carefully:</strong> This applicant meets minimum requirements but may need additional verification or conditions. Review weak areas before deciding.
            </span>
          )}
          {application.score < 60 && (
            <span className="text-red-800">
              <strong>Not Recommended:</strong> This applicant falls below standard qualification criteria. Consider requesting additional information or co-signer before approval.
            </span>
          )}
        </p>
      </div>
