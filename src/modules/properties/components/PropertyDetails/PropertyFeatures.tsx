    ...
    <div className="bg-white rounded-lg shadow p-6">
                ...
                {features.map((feature) => (
                  <li key={feature} className="flex items-start">
                    <span className="text-green-500 mr-2 mt-0.5">
                      {featureIcons[feature] || <CheckCircle className="w-5 h-5" />}
                    </span>
      ...
      <div className="mt-8 pt-6 border-t">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <DetailItem 
            label="Parking" 
            value={property.details.parking?.spaces ? 
              `${property.details.parking.spaces} ${property.details.parking.type}${property.details.parking.spaces > 1 ? 's' : ''}` : 
              'None'} 
          />
          <DetailItem 
            label="Laundry" 
            value={property.details.laundry ? 
              property.details.laundry.split('_').map(word => 
          ...
          <DetailItem 
            label="Cooling" 
            value={property.details.cooling ? 
              property.details.cooling.split('_').map(word => 
                word.charAt(0).toUpperCase() + word.slice(1)
              ).join(' ') : 
              'None'} 
          />
          <DetailItem 
            label="Heating" 
            value={property.details.heating ? 
              property.details.heating.split('_').map(word => 
                word.charAt(0).toUpperCase() + word.slice(1)
              ).join(' ') : 
              'None'} 
          />
