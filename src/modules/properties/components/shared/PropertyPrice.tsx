...
export const PropertyPrice: React.FC<PropertyPriceProps> = ({
  ...
  const priceChange = originalPrice
    ? priceCalculator.calculatePriceChange(originalPrice, price)
    : 0;
...
export const PropertyPriceDetailed: React.FC<PropertyPriceDetailedProps> = ({
  ...
  const moveInCost =
    listingType === "rent"
      ? priceCalculator.calculateMoveInCost(
          price,
          deposit,
          0,
          applicationFee,
          ...
          {listingType === "rent" ? (
            ...
            <>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-900">
                  {priceCalculator.formatCurrency(
                    priceCalculator.estimateClosingCosts(price).min,
                  )}{" "}
                  -
                  {priceCalculator.formatCurrency(
                    priceCalculator.estimateClosingCosts(price).max,
                  )}
                </span>
              </div>
...
export const PropertyPriceComparison: React.FC<
  PropertyPriceComparisonProps
> = ({ price, averagePrice, medianPrice, listingType, className = "" }) => {
...
export const PropertyPriceHistory: React.FC<PropertyPriceHistoryProps> = ({
  ...
  const priceChange = priceCalculator.calculatePriceChange(
    initialPrice,
    currentPrice,
  );
