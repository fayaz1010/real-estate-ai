...
export interface MortgageCalculationInput {
  ...
  export const priceCalculator = {
    ...
    calculateMortgage: (input: MortgageCalculationInput): MortgageCalculationResult => {
      ...
      const downPaymentAmount = input.downPaymentType === 'percentage'
    ...
    calculateAffordability: (input: AffordabilityInput): AffordabilityResult => {
      ...
      const debtToIncomeRatio = ((input.monthlyDebts + maxMonthlyPayment) / monthlyIncome) * 100;
     ...
     */
    calculateRentToPriceRatio: (monthlyRent: number, homePrice: number): number => {
      const annualRent = monthlyRent * 12;
