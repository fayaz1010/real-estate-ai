...
export const propertyValidation = {
  ...
  validatePricing: (
    ...
    if (listingType === "rent") {
      if (price < 100) {
        return {
          valid: false,
          error: "Monthly rent seems too low. Please verify.",
        };
      }
      if (price > 50000) {
        return {
          valid: false,
          error: "Monthly rent seems too high. Please verify.",
        };
      }
    } else {
      if (price < 10000) {
        return {
          valid: false,
          error: "Sale price seems too low. Please verify.",
        };
      }
      if (price > 100000000) {
        return {
          valid: false,
          error: "Sale price seems too high. Please verify.",
        };
      }
    }
  ...
  validateImages: (images: File[]): { valid: boolean; errors: string[] } => {
    ...
    images.forEach((file, index) => {
      ...
      if (!allowedFormats.includes(file.type)) {
        errors.push(
          `File ${index + 1}: Only JPEG, PNG, and WebP formats are allowed`,
        );
      }
    });
  ...
  validatePropertyForm: (
    ...
    if (step === 1 || !step) {
      // Basic Details
      const titleValidation = propertyValidation.validateTitle(
        formData.title || "",
      );
      if (!titleValidation.valid) errors.title = titleValidation.error!;

      const descValidation = propertyValidation.validateDescription(
    ...
    if (step === 2 || !step) {
      // Address
      if (formData.address) {
        const addressValidation = propertyValidation.validateAddress(
          formData.address,
        );
    ...
    if (step === 6 || !step) {
      ...
      if (formData.images && formData.images.length > 0) {
        const imageValidation = propertyValidation.validateImages(
          formData.images,
        );
