...
class PropertyService {
  ...
  async getProperties(filters?: SearchFilters): Promise<PropertiesResponse> {
    ...
    if (filters) {
      ...
      if (filters.propertyType) {
        filters.propertyType.forEach(type => queryParams.append('propertyType', type));
      }
   ...
   * Create new property
   */
  async createProperty(propertyData: Partial<PropertyFormData>): Promise<Property> {
   ...
   */
  async getSimilarProperties(id: string, limit: number = 6): Promise<Property[]> {
    const response = await fetch(
   ...
   */
  async getMyProperties(page: number = 1, limit: number = 20): Promise<PropertiesResponse> {
    const response = await fetch(
   ...
   */
  async contactOwner(propertyId: string, message: {
    name: string;
    email: string;
    phone?: string;
    message: string;
   ...
   */
  async scheduleTour(propertyId: string, tourData: {
    date: string;
    time: string;
    name: string;
    email: string;
